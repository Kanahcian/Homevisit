  ...
  useEffect(() => {
    ...
    const searchLocations = (searchQuery) => {
      ...
      const exactMatches = locations.filter(location => {
        ...
        return name === searchQuery || name.startsWith(searchQuery) || name.includes(searchQuery);
      });
      ...
      const results = locations
        .map(location => {
          // 計算相似度分數
          const score = similarityScore(location.name.toLowerCase(), searchQuery);
          
          // 檢查是否為包含匹配
          const isContained = location.name.toLowerCase().includes(searchQuery);
          
          // 計算匹配位置 (用於排序和顯示高亮)
          const matchPosition = location.name.toLowerCase().indexOf(searchQuery);
